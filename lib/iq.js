...
/* Global common pipeline elements */
let decomposer, composer, input;
/**** API ****/
...
function listen(port) {
  return new Promise((resolve, reject) => {
    ...
    server.on('connection', (socket) => {
      socket.pipe(decomposer);
      composer.pipe(socket);
      socket.on('error', (err) => {
        reject(err);
      });
      socket.on('close', () => {
        socket.unpipe(decomposer);
        composer.unpipe(socket);
      });
    });
  });
}
...
/* Construct common pipeline */
decomposer = new Msg.decomposer();
composer = new Msg.composer();
input = new Input();
decomposer.pipe(input);
output.pipe(composer);

module.exports.listen = listen;
module.exports.connect = connect;
module.exports.on = conveyor.subscribe.bind(conveyor);
module.exports.off = conveyor.unsubscribe.bind(conveyor);
