...
const DISCONNECT = {type: 'IQ', action: 'IQ-DISCONNECTED'};
const DISCONNECT_REGEX = /IQ-DISCONNECTED/;
/**** API ****/
...
function Server (options = {}) {
  ...
  this.listen = function listen(port = 'iidk') {
...
function Client (pipeOpts = {}) {
  const pipeline = new Pipeline(pipeOpts);
  const conveyor = pipeline.conveyor;
  const DEFAULT_OPTS = {
    ip: '127.0.0.1',
    port: 'iidk',
  ...
  this.connect = function connect(options = DEFAULT_OPTS) {
    return new Promise((resolve, reject) => {
      const socket = net.Socket();
      const ip = options.ip;
      const port_name = options.port;
      const port = PORTS[port_name];
      const host_id = options.host || os.hostname().toUpperCase();
      const reconnect = options.reconnect;
      const iidk_id = options.iidk ? '.' + options.iidk : '';
      const iqConnected = {
        type: 'IQ',
        action: 'IQ-CONNECTED',
      };
      const iqDisconnected = {
        type: 'IQ',
        action: 'IQ-DISCONNECTED',
      };
      const disconnectSub = pipeOpts.regex ? DISCONNECT_REGEX : DISCONNECT;
      ...
      conveyor.subscribe(disconnectSub, () => socket.end());
      connectSocket();
    });
  };
...
function Input(options) {
  ...
  this.conveyor = new Conveyor(options.regex);
